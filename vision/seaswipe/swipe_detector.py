...
# ── Tuning constants ───────────────────────────────────────────────────────
SPEED_THRESHOLD = 14   # px/frame  — min wrist speed to start a burst
DIST_THRESHOLD  = 50   # px        — min net X travel to fire a swipe
PEAK_SPEED_MIN  = 18   # px/frame  — peak speed needed to confirm intent
COOLDOWN_FRAMES = 20   # frames    — lock-out after a swipe fires
ARM_ANGLE_MAX   = 150  # degrees   — elbow→wrist angle limit
TORSO_MOVE_MAX  = 15   # px        — max shoulder-midpoint drift during burst
Y_BAND_PX       = 40   # px        — wrist must stay within ±40px of swipe-start Y;
