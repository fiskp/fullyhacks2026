...
The animal dataset lives in the **Human Delta virtual filesystem** at `/agent/depth-charge/animals.json`. `build_dataset.py` writes it there once; the game reads it at runtime via the HD FS API — no local JSON file is bundled.
...
3. For each of 60 hardcoded animal names, calls `POST /v1/search` for weight data,
   ...
   then writes the curated array to `POST /v1/fs { op: "write", path: "/agent/depth-charge/animals.json" }`.

### `game/game.js`
On page load, calls `POST /v1/fs { op: "read", path: "/agent/depth-charge/animals.json" }` to
